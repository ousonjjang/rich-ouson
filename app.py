...
if cost > 0:
    ...
    with col2:
        st.metric(label="📉 10년 뒤 잃게 될 자산 (복리 8%)", value=f"{int(future_value):,} 원")
    ...
    chart_data = pd.DataFrame({
        "년차": range(1, 11),
        "투자했을 때 (자산)": [cost * ((1 + 0.08) ** i) for i in range(1, 11)],
        "그냥 뒀을 때 (현금)": [cost for _ in range(1, 11)]
    })
    st.line_chart(chart_data, x="년차", y=["투자했을 때 (자산)", "그냥 뒀을 때 (현금)"], color=["#FF0000", "#CCCCCC"])
