...
if cost > 0:
    ...
    # 클릭 유도 멘트 추가 (체류시간 확보)
    st.markdown("---")
    st.write("🧐 **잠깐! 부자들은 어떤 기준으로 소비할까요?**")
    st.caption("👇 아래를 클릭해서 '절대 소비 기준'을 확인하고 당신의 등급을 체크해보세요.")

    # 등급 기준표 보여주기 (Expander)
    with st.expander("📊 소비 등급 기준표 보기 (클릭해서 펼치기)"):
        st.markdown("""
        ### 👑 당신의 소비 계급은?
        
        * 💀 **노비 (Danger):** 노동 시간 **3시간 초과**
            * 💬 *"주인님, 오늘 오전 일당은 이걸로 다 썼습니다..."*
        * 🤔 **평민 (Warning):** 노동 시간 **1시간 ~ 3시간**
            * 💬 *"음, 밥 한 끼 값 치고는 좀 노동이 많이 들어가는데?"*
        * ✅ **양반 (Safe):** 노동 시간 **1시간 미만**
            * 💬 *"이 정도는 내 시급으로 가볍게 커버 가능하오!"*
            
        ---
        **💡 부자가 되는 습관**
        금액이 아니라 **'내 시간'**을 기준으로 소비를 결정하세요. 
        """)
